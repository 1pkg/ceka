...
contract('CEKA', async (accounts) => {
	contract('Constraints', async (accounts) => {
		...
		it('should not accept put time delta constraint', async () => {
			...
			assert.equal(await ceka.amntCurrent.call(), 10 * FINNEY + 50 * SZABO)
		...
		it('should not accept get before finish', async () => {
			...
			assert.equal(await ceka.amntCurrent.call(), 10 * FINNEY + 100 * SZABO)
		})
	...
	contract('Proceed', async (accounts) => {
		...
		it('should correctly proceed single put', async () => {
			...
			assert.equal(await ceka.amntCurrent.call(), 10 * FINNEY + 450 * SZABO)
		...
		it('should correctly proceed single leave', async () => {
			...
			// check props
			assert.equal(await ceka.amntInit.call(), 10 * FINNEY)
			assert.equal(await ceka.amntTotal.call(), 10 * FINNEY + 100 * FINNEY)
			assert.equal(await ceka.amntClean.call(), 10 * FINNEY + 50 * FINNEY)
			assert.equal(await ceka.amntCurrent.call(), 10 * FINNEY + 50 * FINNEY)
		...
		it('should correctly proceed several leave', async () => {
			...
			assert.equal(await ceka.amntClean.call(), 10 * FINNEY + 225 * FINNEY)
			assert.equal(await ceka.amntCurrent.call(), 10 * FINNEY + 225 * FINNEY)
		...
		it('should correctly proceed single get', async () => {
			...
			let initBalanceMaster = 1 * (await web3.eth.getBalance(master.address))
			...
			let curBalanceMaster = 1 * (await web3.eth.getBalance(master.address))
			assert.approximately(
				initBalanceMaster,
